...
func TestLocationService_GetLocationById(t *testing.T) {
	type args struct {
		ctx    context.Context
		client any
		id     int
	...
	testCases := []struct {
	...
	}{
		{
			...
			mockBehavior: func(m *mocks.MockLocationRepo, args args) {
				m.EXPECT().GetLocationById(args.ctx, args.client, args.id).
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// run test
			got, err := s.GetLocationById(tc.args.ctx, tc.args.client, tc.args.id)
			if tc.wantErr {
				assert.Error(t, err)
				return
			}
