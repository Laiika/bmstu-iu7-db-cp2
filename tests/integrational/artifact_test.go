...
func TestPgArtifactService_GetArtifactById(t *testing.T) {
	type args struct {
		ctx    context.Context
		client any
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.wantErr {
				_, err := tc.s.GetArtifactById(tc.args.ctx, tc.args.client, 1)
				assert.Error(t, err)
				return
			}
			...
			id, err := tc.s.CreateArtifact(tc.args.ctx, tc.args.client, tc.args.input)
			assert.NoError(t, err)
			tc.want.Id = id

			got, err := tc.s.GetArtifactById(tc.args.ctx, tc.args.client, id)
			assert.NoError(t, err)
