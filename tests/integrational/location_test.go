...
func TestPgLocationService_GetLocationById(t *testing.T) {
	type args struct {
		ctx    context.Context
		client any
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.wantErr {
				_, err := tc.s.GetLocationById(tc.args.ctx, tc.args.client, 1)
				assert.Error(t, err)
				return
			}
			...
			got, err := tc.s.GetLocationById(tc.args.ctx, tc.args.client, id)
			assert.NoError(t, err)
