...
func TestPgEquipmentService_GetEquipmentById(t *testing.T) {
	type args struct {
		ctx    context.Context
		client any
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.wantErr {
				_, err := tc.s.GetEquipmentById(tc.args.ctx, tc.args.client, 1)
				assert.Error(t, err)
				return
			}
			...
			assert.NoError(t, err)
			tc.want.ExpeditionId = expeditionId
			...
			got, err := tc.s.GetEquipmentById(tc.args.ctx, tc.args.client, id)
			assert.NoError(t, err)
